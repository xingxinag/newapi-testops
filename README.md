...
| 变量 | 作用 |
| --- | --- |
| `API_HOST` | API 后端监听地址。本地常用 `127.0.0.1`，Docker 或服务器公开监听常用 `0.0.0.0`。 |
| `API_PORT` | API 后端端口，默认 `8788`。 |
| `WEB_HOST` | Node 开发 Web 服务监听地址，默认 `127.0.0.1`。 |
| `WEB_PORT` | Node 开发 Web 服务端口，默认 `4178`。 |
| `DATA_DIR` | 任务、账号、会话、团队等 JSON 数据目录。 |
| `AUTH_REQUIRED` | 是否要求登录后才能访问 jobs、schedules、artifacts、exports、analytics 等接口。 |
| `ARTIFACT_LOCAL_DIR` | 本地 artifact 保存目录。 |
| `SCHEDULER_POLL_INTERVAL_MS` | worker 轮询调度的间隔。 |
...
`npm run build` 会运行 `scripts/build.mjs`。构建脚本会做三件事：

1. 删除旧的 `dist/web`。
2. 复制 `apps/web/index.html` 和 `apps/web/src` 到 `dist/web`。
3. 生成 `dist/web/config.js`。

`config.js` 的内容来自环境变量 `NEWAPI_TESTOPS_API`。如果你没设置它，就使用默认值：
...
也就是说，静态前端访问哪个 API，是构建时写进 `dist/web/config.js` 的。你把 `dist/web` 上传到 Vercel、Cloudflare Pages、GitHub Pages、Nginx 或对象存储后，浏览器会读取这个 `config.js`。

重要区别：

| 场景 | API 地址来自哪里 |
| --- | --- |
| `npm run start:web` 开发模式 | Web 服务直接服务 `apps/web`，前端通常使用默认 API 地址。 |
| `npm run build` 静态部署 | `dist/web/config.js` 里的 `window.__NEWAPI_TESTOPS_API__`。 |
| Docker Web 镜像 | 镜像里打包的 `dist/web/config.js`。如果构建前没设置 `NEWAPI_TESTOPS_API`，默认就是 `http://127.0.0.1:8788`。 |
